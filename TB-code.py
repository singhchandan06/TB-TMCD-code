...
def kpath(a):
    N= 100
    kx1=linspace(0,4*pi/(3*a),N)
    kx2=linspace(4*pi/(3*a),pi/a,N)
    kx3=linspace(pi/a,2*pi/(3*a),N)
    kx4=linspace(2*pi/(3*a),0,N)
    kx = concatenate((kx1,kx2,kx3,kx4))
    ky1=linspace(0,0,N)
    ky2=linspace(0,pi/(sqrt(3)*a),N)
    ky3=linspace(pi/(sqrt(3)*a),2*pi/(sqrt(3)*a),N)
    ky4=linspace(2*pi/(sqrt(3)*a),0,N)
    ky = concatenate((ky1,ky2,ky3,ky4))
    return kx,ky
# Write the tight-binding Hamitlonian with hopping and onsite energy terms as parameter
...
t12 = 0.2787; t22 = -0.0539;
r0 = 0.00037;  r1 = -0.0997;
r2 = 0.0385; r11 = 0.032;
r12 = 0.0986; u0 = 0.0685;
u1 = -0.0381; u2 = 0.0535;
u11 = 0.0601; u12= -0.0179;
u22 = -0.0425;
E1 = 1.36; E2 =  1.67
lam = 0.0784
...
ssp = zeros([6,nk])
kk = []
# call the function to calculate the eigenvalues and stored in ssp
for i in arange(nk):
    kx = kx0[i] ; ky = ky0[i]
    k  = sqrt((kx0[i]-kx0[i-1])**2+(ky0[i]-ky0[i-1])**2)
    kk.append(k)
    ...
    evals = sorted(evals)
    ssp[:,i] = evals
#plot the band structure and compare with DFT band structure of MoS2 monolayer
X0 = kp[0]/kp[399]
X1 = kp[99]/kp[399]
X2 = kp[199]/kp[399]
X3 = kp[299]/kp[399]
...
ax.set_xticklabels(['$\Gamma$', 'K', 'M', "K'", '$\Gamma$'], fontsize=20)
#ax.set_yticklabels(fontsize=20)


#plotting
df1=np.loadtxt('band25.dat')
df2=np.loadtxt('band26.dat')
plt.plot(df1[:,0]/0.575587, df1[:,2]+ef, color='red', linewidth=1.5, label='DFT')
plt.plot(df2[:,0]/0.575587, df2[:,2]+ef, color='red', linewidth=1.5)
plt.plot(kp/kp[399], ssp[0,:], 'blue', label='TB-model', linewidth=1.5)
plt.plot(kp/kp[399], ssp[1,:], 'blue', linewidth=1.5)
